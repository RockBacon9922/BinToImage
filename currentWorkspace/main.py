...
if len(sys.argv) != 2:
    print("Usage: python program.py binary_file")
    sys.exit(1)
...
with open(filename, "rb") as file:
    ...
    # Read the image data from the binary file into a numpy array
    image_data = np.frombuffer(file.read(), dtype=np.uint8)


# get length of image data then work out width and height from aspect ratio
length = len(image_data)
width = int(math.sqrt(length / 3))
height = int(length / (width * 3))

# get product of width and height times it by 3 then split the image data after that point
