...
colour_map = [0x000000, 0x560000, 0x640000, 0x750000, 0x870000, 0x9b0000, 0xb00000, 0xc60000, 0xdd0000, 0xf50000, 0xff0f0f, 0xff2828, 0xff4343, 0xff5e5e, 0xff7979, 0xfe9595,
...
if len(sys.argv) != 3:
    print("Usage: python program.py binary_file colour_depth")
    sys.exit(1)

filename = sys.argv[1]
colour_depth = int(sys.argv[2])

with open(filename, "rb") as file:
    # Read the image data from the binary file
    image_data = file.read()

# Calculate the width and height from the length of the image data
length = len(image_data)
width = int(math.sqrt(length / 3))
height = int(length / (width * 3))

# Reshape the image data into a 3D array of RGB values
pixels = np.frombuffer(image_data[:width * height * 3], dtype=np.uint8)
pixels = pixels.reshape(height, width, 3)

# Create a lookup table to map RGB values to color indexes
lut = np.zeros((256, 256, 256), dtype=np.uint8)
for i, color in enumerate(colour_map):
    r = (color >> 16) & 0xff
    g = (color >> 8) & 0xff
    b = color & 0xff
    lut[r, g, b] = i

# Map the RGB values to color indexes using the lookup table
pixels = lut[pixels[..., 0], pixels[..., 1], pixels[..., 2]]
...
# Convert the image to the desired colour depth
img = img.convert("P", palette=Image.ADAPTIVE, colors=2 * colour_depth)
